...
# xn_dens = readData('x_numDensity.txt')
# yn_dens = readData('y_numDensity.txt')
...
#     s_2 = math.sqrt(2)   
    print(.5*boxL/delta_r)
    r_vec = np.linspace(0,boxL/2.0,boxL/(2.0 * delta_r) + 1.0)
    print("length of r_vec is",len(r_vec))
...
#     r_new = np.linspace(0,boxL/2.0,(boxL/(2.0 * delta_r) + 1.0)*3)
#     spl = make_interp_spline(r_vec,RDF,k=1)
#     psmooth = spl(r_new)
#     plt.plot(r_new,psmooth)
#     plt.show()
...
def run_pcf():
    ...
    cell_L = sigma/20.0
    delta_r = sigma/15.0
    print(delta_r,"and",boxL)
    val = int(math.sqrt(2)*boxL/cell_L) + 1
#     init_pos_matrix(val,cell_L)
#     print(n,val)
