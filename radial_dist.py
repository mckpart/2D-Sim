...
# calculates the 'missing' parameter
if(boxL == 0): 
...
# caluclated the RDF using a radial number density
for k in range(num):
...
# takes the 1D arrays and converts them back
# into 2D arrays
...
# creates the 1D radial number density from the 
# x and y pair correlations
...
# calculates the radial distribution function using the 
# radial number density extracted from the pair 
...
# runs the code associated with the pair correlation function
def run_pcf():
